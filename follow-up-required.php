...
// Retrieve the total number of follow-up required leads
$total_stmt = $conn->prepare("SELECT COUNT(*) as total FROM clients WHERE lead_status = 'Follow Up Required'");
$total_stmt->execute();
...
// Retrieve follow-up required leads with pagination, ordered by next_follow_up date
$stmt = $conn->prepare("SELECT * FROM clients WHERE lead_status = 'Follow-Up Required' ORDER BY next_follow_up DESC LIMIT ?, ?");
$stmt->bind_param("ii", $start, $limit);
$stmt->execute();
$result = $stmt->get_result();

// Get the current date for overdue/ upcoming calculations
...
<div class="d-flex" id="wrapper">
    ...
    <div id="page-content-wrapper">
        <div class="container-fluid">
            <h1 class="mt-4">Follow Up Required</h1>
            <div class="mb-3">
                <form method="GET" action="follow-up-required.php" class="form-inline">
                ...
                <nav>
                    <ul class="pagination">
                        <li class="page-item <?php if($page <= 1){ echo 'disabled'; } ?>">
                            <a class="page-link" href="<?php echo ($page <= 1 ? '#' : "?limit=$limit&page=".($page - 1)); ?>">Previous</a>
                        </li>
                        <?php for($i = 1; $i <= $total_pages; $i++ ): ?>
                        <li class="page-item <?php echo ($page == $i ? 'active' : ''); ?>">
                            <a class="page-link" href="follow-up-required.php?limit=<?php echo $limit; ?>&page=<?php echo $i; ?>"><?php echo $i; ?></a>
                        </li>
                        <?php endfor; ?>
                        <li class="page-item <?php if($page >= $total_pages) { echo 'disabled'; } ?>">
                            <a class="page-link" href="<?php echo ($page >= $total_pages ? '#' : "?limit=$limit&page=".($page + 1)); ?>">Next</a>
                        </li>
                    </ul>
                </nav>
                <div>
